...
import { faUser, faEnvelope, faPhone, faLocationDot, faWeightScale, faRulerVertical, faJoint, faMaskFace,faRectangleList, faGraduationCap, faAward, faUserDoctor, faHouseChimneyMedical } from '@fortawesome/free-solid-svg-icons';
...
function UserInfo({ userId, role }: { userId: string; role: string | null }) {
  ...
  useEffect(() => {
    if (role === "patient") {
      const fetchPhistory = async () => {
        try{
          const url = `https://medica.runasp.net/GetPhistory?pid=${userId}`;
          const response = await fetch(url);
           const data = await response.json();
  ...
  useEffect(() => {
    if (role === "doctor"){
      const fetchDoctorCV = async () => {
        try{
          const url = `https://medica.runasp.net/api/Doctor/GetDoctorCV?did=${userId}`;
          const response = await fetch(url);
          const data = await response.json();
          ...
          console.error('Error fetching doctor CV:', error);
      }
    }
    fetchDoctorCV();};
    },[]);
  ...
  return (
    <>
     {/* floating box */}
    <div className="group  transition-all duration-300 h-[850px] shadow-2xl rounded-3xl 
                     px-6 py-4 bg-gradient-to-r from-sky-900 to-indigo-900 w-full relative">
        ...
        {/* personal info content */}
         <div className={`p-4 flex flex-row flex-wrap justify-between gap-y-10  rounded-3xl
            ...
            <div className="basis-[40%] 
                            transition-translate ease-out duration-100  translate-y-0 -translate-y-[150%]
                            ">
                <div className="flex flex-row items-center gap-x-2">
                    <FontAwesomeIcon icon={faUser} className="text-[#fdf0d5] text-2xl inline mr-1" /> 
                    <h1 className=" text-2xl text-[#fdf0d5]">Name</h1>
                </div>
                <p className=" text-[#fdf0d5] ">{user.firstname} {user.lastname}</p>
            </div>
            ...
            <div className="basis-[40%]
                ...
                <p className=" text-[#fdf0d5] ">{user.email}</p>
            </div>
            ...
            <div className="basis-[40%]
                ...
                <p className= "text-[#fdf0d5] ">{user.phone}</p>
            </div>
            ...
            <div className="basis-[40%]
                ...
                <p className=" text-[#fdf0d5]">{user.address}</p>
            </div>
            ...
            <div className="basis-[40%]
                            ...
                            ">
                <div className="flex flex-row items-center gap-x-2">
                    <FontAwesomeIcon icon={faRectangleList} className="text-[#fdf0d5] text-2xl inline mr-1" />
                    <h1 className=" text-2xl text-[#fdf0d5]">Summary</h1>
                </div>
                <p className=" text-[#fdf0d5] mt-2">{summary} </p>
                </div>
            ...
            <div className="basis-[40%]
                            ...
                            ">
                <div className="flex flex-row items-center gap-x-2">
                    <FontAwesomeIcon icon={faAward} className="text-[#fdf0d5] text-2xl inline mr-1" />
                    <h1 className=" text-2xl text-[#fdf0d5]">Certifications</h1>
                </div>
                <p className=" text-[#fdf0d5] mt-2">{certification}</p>
                </div>
         ...
         {/* links */}
         <div className={`my-8 flex flex-row justify-center gap-x-3 
                          transition-translate ease-out duration-300 delay-200 translate-y-0 
                          transition-opacity
                          max-[] w-full mx-auto absolute bottom-10`}>
            {/* Facebook Link */}
            {user.facebook && (
